...
const GamePage = () => {
   ...
   useEffect(() => {
      const fetchRandomLocations = async () => {
         try {
            const randomLocations = await getRandomLocationsApi(selectedMode, amountOfRounds);
   ...
   const handleResult = (newDistance, newShowResult) => {
      ...
      const pointsForRound = calculatePoints(
         newDistance,
         maxPointsPerRound,
         rounds
      );
   ...
   const coordOfCurrentPlace = currentLocation
   ...
   return (
      <div className="g_page_main_container">
         <StreetView
            latitude={coordOfCurrentPlace[0]}
            longitude={coordOfCurrentPlace[1]}
         />
         <div className="g_page_current_points">
            {currentPoints.toFixed(0)}/{MAX_POINTS_PER_GAME}
            <div>
               {" "}
               current round: {currentRound + 1}/{amountOfRounds}
            </div>
         </div>
